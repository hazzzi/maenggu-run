 ...
 */
export async function loadSpritePack(basePath: string): Promise<SpritePackLoadResult> {
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, error: `Failed to fetch manifest: ${response.status}` }
    }
 ...
 * 상태가 없으면 fallback 사용
 */
export function getFrameUrls(pack: SpritePack, state: string): readonly string[] {
  const config = pack.manifest.states[state] ?? pack.manifest.states[pack.manifest.fallback]
 ...
 * 상태가 없으면 fallback 사용
 */
export function getStateConfig(pack: SpritePack, state: string): SpriteStateConfig {
  return pack.manifest.states[state] ?? pack.manifest.states[pack.manifest.fallback]
}
